...
public class CursoServiceImpl implements ICursoService {
    ...
    @Override
    @Transactional
    public CursoResponseDTO create(CursoRequestDTO requestDTO, List<MultipartFile> files) {

        // 1.- Mappear el DTO a la entidad
        CursoModel curso = cursoMapper.toEntity(requestDTO);

        // 2.- Procesar galería si existen imágenes
        if (files != null && !files.isEmpty()) {

            // Validación de cantidad
            if (files.size() > 11) {
                throw new IllegalArgumentException("No se pueden subir más de 11 imágenes (1 portada + 10 galería).");
            }

            for (int i = 0; i < files.size(); i++) {
                MultipartFile image = files.get(i);

                // Validar que el archivo no sea el "empty" que manda el DataProvider o un
                // archivo corrupto
                if (image != null && !image.isEmpty() && image.getSize() > 0
                        && !image.getOriginalFilename().equals("empty")) {

                    // Subimos la imagen. i será la posición (0 para portada, 1+ para galería)
                    CursoImageModel imageModel = uploadImage(image, i);

                    // Sincronización bidireccional (esto la agrega a la lista curso.getImages())
                    curso.addImage(imageModel);

                    // ASIGNACIÓN CRÍTICA: La primera imagen (índice 0) es la portada
                    if (i == 0) {
        ...
        // Es vital guardar el curso completo; el CascadeType.ALL se encargará de las
        // imágenes.
        CursoModel savedCurso = cursoRepository.save(curso);
    ...
    public CursoResponseDTO update(Long id, CursoRequestDTO requestDTO,
        ...
        List<Long> keepingImageIds = requestDTO.getKeepImageIds() != null ? requestDTO.getKeepImageIds()
                : new ArrayList<>();
        ...
        // Procesar nuevas imágenes sólo si se proporcionan
        if (files != null && !files.isEmpty()) {

            // Filtrar solo las imágenes válidas (evitamos el "empty" del DataProvider)
            ...
            if (!validGalleryImages.isEmpty()) {
                ...
                // Validar que la cantidad total de imágenes (existentes + nuevas) no exceda el
                // límite
                ...
                for (int i = 0; i < validGalleryImages.size(); i++) {
                    ...
                    if (i == 0 && curso.getCoverImage() == null) {
                        ...
                        int nextPos = curso.getImages().size();
                        curso.addImage(uploadImage(image, nextPos));
                    }
                }
            }
        }
