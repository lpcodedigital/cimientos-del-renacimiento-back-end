...
public class ObraServiceImpl implements IObraService {
    ...
    public List<ObraMapaDTO> findAllForObraMapa() {
        ...
        // 2. Convertimos la lista de Proyecciones a la lista de DTOs que espera tu
        // controlador
    ...
    public ObraResponseDTO findById(Long id) {
        ...
        // Usamos la consulta optimizada que carga las imágenes con JOIN FETCH para
        // evitar el problema N+1 de las imágenes
        ObraModel obra = obraRespository.findByIdWithImages(id)
                .orElseThrow(() -> new ResourceNotFoundException("No se encontró la obra con ID: " + id));
    ...
    public ObraResponseDTO update(Long id, ObraRequestDTO request, List<MultipartFile> newFiles) {
        ...
        imagesToRemove.forEach(img -> {
            ...
            try {
                ...
                // Loguear el error pero continuar con la eliminación si es critico mantener la
                // coherencia
                // Si Cloudflare falla por red o API, solo avisamos en consola
                System.err.println("Inconsistencia: No se borró en la nube, pero se quitará de la DB: "
                        + img.getProviderId() + " - " + e.getMessage());
            }
    ...
    public void delete(Long id) {
        ...
        ObraModel obra = obraRespository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("No se puede eliminar: Obra no encontrada"));
        ...
        obra.getImages().forEach(img -> {
            ...
            // Intentar borrar en Cloudflare mientras recorremos las imágenes, así evitamos
            // tener que hacer un segundo recorrido solo para eso
            try {
                imageStorageService.delete(img.getProviderId());
        ...
        /*
         * Nota: Al ejecutar el delete de la obra automaticamente se marca deleted =
         * true
         * no se elimina fisicamente de la base de datos gracias a la
         * anotación @SQLDelete
         * en la entidad ObraModel y las imagnes se marcan como deleted = true pero no
         * se puede
         * tener una audtioria para saber quien hizo ese cambio en el estado de las
         * imagenes.
         */
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<ObraResponseListDTO> findAllPaginated(int page, int size, String search) {
        ...
        if (search != null && !search.trim().isEmpty()) {
            ...
            formatedSearch = Arrays.stream(search.trim().split("\\s+"))
                    ...
                    .collect(Collectors.joining(" & ")); // Combina los términos con el operador AND para la consulta de texto completo
        }

        // Llamamos a la consulta nativa con FTS
        Page<ObraPaginationProjection> projections = obraRespository.findAllByFullTextSearch(formatedSearch, pageable);

        // Convertimos la página de proyecciones a página de DTOs
        // Como no queremos imágenes, el campo 'images' en ObraResponseListDTO quedará
        // como null, lo cual es correcto para esta vista de paginación

        return projections.map(obraMapper::projectionToResponseDTO);
